...
object base_pair {
  ...
  val conf:SparkConf = new SparkConf().setAppName("base_pair").setMaster("local")
  ...
  def createPairMap():Unit = {
     ...
     *
     * */
    val rddFile:RDD[(String,String)] = sc.textFile("file:///D:/Workspaces/IdeaProjects/SCALA_PROJECTS/data/spark_test_0.1/in/data_pair.txt", 1).map { x => (x.split(",")(0),x.split(",")(1) + "," + x.split(",")(2)) }
  ...
  def pairMapRDD(path:String):Unit = {
    ...
    println("====rddSortAsc=====:" + rddSortAsc.collect().mkString(","))// (k01,3),(k01,26),(k02,6),(k03,2) 注意key相同的内部value不会排序
